(function() {
    ...
    angular
    .module('aui.core.resources')
    .factory('commonFunctionService', commonFunctionService);

    commonFunctionService.$inject = ['ruleService', 'LinkAniService', 'RULE_INFO', 'POLICY_TYPE', '$log', 'SaveConfirmUtilService', '$translate', 'DialogService', '$location', '$filter', 'chineseWallService', 'setResource', 'mailDomainResource', 'employeeResource', '$cookies'];

 function commonFunctionService(ruleService, LinkAniService, RULE_INFO, POLICY_TYPE, $log, SaveConfirmUtilService, $translate, DialogService, $location, $filter, chineseWallService, setResource, mailDomainResource, employeeResource, $cookies) {
    ...
    /** 
        getRuleService : 시브 룰 조회용 공통함수
            serviceName : 서비스명(자동분류, 사용자라우팅룰, 트랜스포트룰 서브미션룰), constant.module.js 참조
            targetId : 계정정보, 자동분류와 사용자 라우팅룰은 개별 계정 정보가 필요함
            currentScope : 공통함수 서비스 함수가 개별 컨트롤러에게 받아서 처리하고 다시 반환할 컨트롤러의 $scope
    */
    commonFunctionService.getRuleService = function(serviceName, targetId, currentScope) {
        ruleService[serviceName.GET](targetId).$promise.then(function (data) {
    ...
    */
    ...
    /** 
        updateRuleServiceForSystem : 시스템용 룰 업데이트
    ...
    */
    function updateRuleServiceForSystem(serviceName, currentScope) {
        var beforeFnlMdfyTs = undefined;
        var currentFnlMdfyTs = currentScope.sieveListModelOrigin.fnlMdfyTs
        ruleService[serviceName.GET]().$promise.then(function (data) {
    ...
    /** 
        updateRuleServiceForUser : 유저용 룰 업데이트
            수정 직전에 시브 룰 데이터를 다시 조회해서 처음 데이터를 조회했을때와 시간이 변경되었는지 확인,
            변경되었다면 누군가 그 사이에 수정한 것 이므로 업데이트 액션을 중지시키고, 팝업을 띄움
            기타 UI작업 수행
    */
    ...
    function OpenSieveRuleSaveDialog(serviceName) {
        if(serviceName.GET === 'getAutoClf')
            SaveConfirmUtilService.open($translate.instant('setting_one.config.sieve.save_auto_classify'), $translate.instant('common.label.save'));
        else if(serviceName.GET === 'getUserRoutingRule')
            SaveConfirmUtilService.open($translate.instant('setting_one.config.sieve.save_user_routing'), $translate.instant('common.label.save'));
        else if(serviceName.GET === 'getTransportRule')
            SaveConfirmUtilService.open($translate.instant('setting_one.config.sieve.save_transport'), $translate.instant('common.label.save'));
        else if(serviceName.GET === 'getSubmissionRule')
            SaveConfirmUtilService.open($translate.instant('setting_one.config.sieve.save_submission'), $translate.instant('common.label.save'));
        else if(serviceName.GET === 'getAutoForwarding')
            SaveConfirmUtilService.open($translate.instant('setting_one.config.sieve.save_auto_forward'), $translate.instant('common.label.save'));
        else {
    ...
    function OpenSieveRuleAlertDialogAndReloading(serviceName, fnlMdfrId, currentScope) {
        ...
        if(serviceName.GET === 'getAutoClf')
            title = 'setting_one.config.sieve.save_auto_classify'
        else if(serviceName.GET === 'getUserRoutingRule')
            title = 'setting_one.config.sieve.save_user_routing'
        else if(serviceName.GET === 'getTransportRule')
            title = 'setting_one.config.sieve.save_transport'
        else if(serviceName.GET === 'getSubmissionRule')
            title = 'setting_one.config.sieve.save_submission'
        else {
            title = 'alert'
    ...
    /**
        buildUpdateRuleData : 시브 룰 업데이트용
    */ 
    function buildUpdateRuleData(currentScope) {
        ...
        var autoClfsCopiedOrigin = undefined;
        var autoClfsCopied = undefined;
    ...
    var externalAutoRequestData = undefined;
    var domainListData = undefined;
    ...
    var initExternalAutoRequest = function(setData){
        ...
        for(var i=0; i<length; i++){
            if(setData.externalAutoRequestControlEnabled.valueList[i].plcyItemValCn == "Y"
                && setData.externalAutoRequestControlEnabled.valueList[i].selected == "Y"){
                enabled = true;
                break;
            }
        }
        ...
        if(type === POLICY_TYPE.USER_LOGIN_BT_POLICY_ORIGINAL) {
            ...
            if(affilationInfo[POLICY_TYPE.SUB] === undefined) {
                ...
                if(lastLoginInfo === POLICY_TYPE.ORIGIN) {
                    ...
                    if(dispAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : false, "desc" : $translate.instant('policy.affiliation.originPrefix') + affilationInfo.O.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.originPolicy'), "alert" : $translate.instant('policy.affiliation.notAuthDescDisp')};
                    }
                    return {"isViewAuth" : false, "isEditAuth" : false, "desc" : "auth failed", "alert" : "auth failed"};
                }
                if(lastLoginInfo === POLICY_TYPE.SUB) {
                    if(upperAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : true, "desc" : $translate.instant('policy.affiliation.dispPrefix') + affilationInfo.S.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.dispPolicy')};
                    }
                    if(originAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : true, "desc" : $translate.instant('policy.affiliation.dispPrefix') + affilationInfo.S.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.dispPolicy')};
                    }
                    if(dispAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : false, "desc" : $translate.instant('policy.affiliation.dispPrefix') + affilationInfo.S.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.dispPolicy'), "alert" : $translate.instant('policy.affiliation.notAuthDescDisp')};
                    }
                    return {"isViewAuth" : false, "isEditAuth" : false, "desc" : "auth failed", "alert" : "auth failed"};
                }
            }
        ...
        if(type === POLICY_TYPE.USER_LOGIN_BT_POLICY_SUB) {
            ...
            if(affilationInfo[POLICY_TYPE.SUB] === undefined) {
                ...
                if(lastLoginInfo === POLICY_TYPE.SUB) {
                    ...
                    if(originAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : false, "desc" : $translate.instant('policy.affiliation.dispPrefix') + affilationInfo.S.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.dispPolicy')};
                    }
                    if(dispAuth === true) {
                        return {"isViewAuth" : true, "isEditAuth" : true, "desc" : $translate.instant('policy.affiliation.dispPrefix') + affilationInfo.S.compName + $translate.instant('policy.affiliation.commonPostfix') + $translate.instant('policy.affiliation.dispPolicy')};
                    }
